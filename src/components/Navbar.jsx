    ...
    >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto '>
          <Link to='/' className='flex items-center gap-2' onClick={() => {setActive(''); window.scrollTo(0, 0)}}>
            <img className='w-12 h-12 invert object-contain' src={logo} alt="logo" />
            <p className='text-white text-[18px] font-bold cursor-pointer flex'>Kowy <span className='sm:block hidden'> XYZ </span></p>
          </Link>
          <ul className='list-none hidden sm:flex flex-row gap-10'>
            {navLinks.map((link, index) => {
              ...
              className={`${active === link.title ? 'text-white' : 'text-secondary'} hover:text-white text-[18px]`} >
                <a className='text-[#fff]' href={`#${link.id}`}>{link.title}</a>
              </li>)
            })}
          </ul>
